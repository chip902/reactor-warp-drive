...
def extract_significant_functions(texts, initial_dictionary):
    ...
    significant_functions = {token: count for token, count in token_counts.items() if count > 1}  # Adjust threshold as needed
...
def main():
    try:
        ...
        if "Action Settings" not in df.columns:
            raise Exception("The CSV file must contain 'Action Settings' column.")
        ...
        significant_functions = extract_significant_functions(action_settings, starting_dictionary)
